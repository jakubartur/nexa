...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog)
{
    ...
    Q_FOREACH (const CAmount &amount, CoinControlDialog::payAmounts)
    {
        ...
        if (amount > 0)
        {
            CTxOut txout(CTxOut::SATOSCRIPT, amount, (CScript)std::vector<unsigned char>(24, 0));
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            {
                CTxOut txout(CTxOut::SATOSCRIPT, nChange, (CScript)std::vector<unsigned char>(24, 0));
                if (txout.IsDust())
                {
